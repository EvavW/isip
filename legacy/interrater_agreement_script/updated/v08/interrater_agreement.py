...
# import classes
import Annotators_annot_v08 as A_ANT
...
def calculate_specificity(Annotator_as_ref_results_dict):
    ...
    for _annotator, ann_results in Annotator_as_ref_results_dict.iteritems():
        ...
        _ann_seiz_FA = float(separate_results[2])
        _ref_bckg_events = float(separate_results[3]) + 1
        _ref_seiz_events = float(separate_results[3])
        _hyp_epochs = float(separate_results[5])
        _miss = float(separate_results[1])
        _hits = float(separate_results[0])


        print _ann_seiz_FA, "#################"
        print _ref_bckg_events, "################"
        ## calculate specificity in percentage
        #
        calculated_specificity = 100* (_hits /(_ref_seiz_events +  _ann_seiz_FA - _miss ))
